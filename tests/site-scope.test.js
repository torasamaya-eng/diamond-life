...
test('サイト整理では保存・共有・広告を含む全ゲームモジュールを変更しない',()=>{
 ...
 for(const [name,hash] of Object.entries(hashes).filter(([name])=>name!=='share.js'))assert.equal(createHash('sha256').update(fs.readFileSync(new URL('../dist/src/'+name,import.meta.url))).digest('hex'),hash,name);
});
